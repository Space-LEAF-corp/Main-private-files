...
def generate_initial_heightmap(width: int, height: int, rng: np.random.Generator) -> np.ndarray:
    ...
    """
    base: np.ndarray = rng.normal(loc=0.0, scale=1.0, size=(height, width))
...
def _convolve2d(arr: np.ndarray, kernel: np.ndarray) -> np.ndarray: # pyright: ignore[reportUnknownParameterType]
