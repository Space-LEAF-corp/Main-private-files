...
class Jarvondis3GlobalInterface:
    def __init__(
        ...
        # consensus_signatures holds signer ids that have successfully provided valid signatures
        self.consensus_signatures: Set[str] = set()
        # trusted_signers maps signer_id -> optional verify_key_b64 (pubkey). None means no pubkey known (HMAC fallback).
        self.trusted_signers: Dict[str, Optional[str]] = {}
    ...
    def add_trusted_signer(self, signer_id: str, verify_key_b64: Optional[str] = None):
        """
        Register a trusted signer. Optionally attach their public verify key (base64).
        If a public key is provided and the crypto adapter supports verification with that key,
        consensus signatures will be verified with it.
        """
        self.trusted_signers[signer_id] = verify_key_b64
    ...
    def provide_consensus(self, signer_id: str, signature: str) -> str:
        ...
        verify_key_b64 = self.trusted_signers.get(signer_id)

        # Message that signers attest to
        message = "CONSENSUS"

        # Prefer crypto adapter verification if available and a verify key is known
        valid = False
        if self.crypto and verify_key_b64:
            # Crypto adapter exposes a stable verify_with_pubkey method
            try:
                valid = self.crypto.verify_with_pubkey(verify_key_b64, message, signature)
            except Exception:
                valid = False

        # If crypto-based verification didn't succeed, fall back to HMAC-based tamper verification
        if not valid:
            try:
                valid = self.tamper.verify_signature(message, signature)
            except Exception:
                valid = False
    ...
    # --- Helper: admin signature verification (supports crypto adapter or HMAC fallback) ---
    def _verify_admin_signature(self, command: str, signature: str) -> bool:
        # If a crypto adapter is configured, attempt to verify using it
        if self.crypto:
            try:
                return self.crypto.verify_admin_signature(command, signature)
            except Exception:
                pass

        # Final fallback: HMAC verification using tamper key (backwards compatibility)
        try:
            return self.tamper.verify_signature(command, signature)
        except Exception:
            return False
    ...
    def grant_emergency_access(self, admin_signature: str, mfa_code: Optional[str] = None) -> str:
        ...
        if not self._verify_admin_signature(command_literal, admin_signature):
    ...
    def request(self, user_role: str, command: str, signature: Optional[str] = None,
        ...
        if is_sensitive:
            if not admin_signature or not self._verify_admin_signature(command, admin_signature):
    ...
    def export_compliance_snapshot(self) -> str:
        snapshot = {
            ...
            "trusted_signers": {k: v for k, v in sorted(self.trusted_signers.items())},
